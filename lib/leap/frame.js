...
var Frame = module.exports = function Frame(frame) {
  this.id = frame.id;
  ...
  for (var i = 0; i < frame.gestures.length; i++) {
    this.gestures.push(new Gesture(frame.gestures[i]));
  }

  for (var i = 0; i < frame.hands.length; i++) {
    this.hands.push(new Hand(frame.hands[i]));
  }

  for (var i = 0; i < frame.pointables.length; i++) {
    this.pointables.push(new Pointable(frame.pointables[i]));
  }
};

Frame.prototype.addObjectRelations = function() {
